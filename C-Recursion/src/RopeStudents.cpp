...
int minDist(struct student *leaderStudent) {
	if (leaderStudent == NULL)return 0;
	return 1 + min(minDist(leaderStudent->first), minDist(leaderStudent->second), minDist(leaderStudent->third));
}
int maxDist(struct student *leaderStudent) {
	if (leaderStudent == NULL)return 0;
	return 1 + max(maxDist(leaderStudent->first), maxDist(leaderStudent->second), maxDist(leaderStudent->third));
}
void findBallDistance(struct student *leaderStudent, int *minBallDistance, int *maxBallDistance)
{
	
	*minBallDistance = minDist(leaderStudent);
	//For this problem i feel like iam confused  for min distance some test cases.
	if (*minBallDistance == 1) {
		if (leaderStudent->first && !leaderStudent->second && !leaderStudent->third)
			;
		else if (!leaderStudent->first && leaderStudent->second && !leaderStudent->third)
			;
		else if (!leaderStudent->first && !leaderStudent->second && leaderStudent->third)
			;
		else if (!leaderStudent->first && !leaderStudent->second && !leaderStudent->third)
			;
		else *minBallDistance = 2;
	}
	*maxBallDistance = maxDist(leaderStudent);
}
